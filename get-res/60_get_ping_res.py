...
def insert_data(res_list):
    ...
    for re in res_list:
        sql = "INSERT INTO `myproject`.`ping_res`(`ping_endpoint`, `ping_ipversion`, `ping_targeturl`, `ping_timestamp`, `ping_value`) VALUES (%s, %s, %s, %s, %s)"
        # cursor.execute(sql,(re['endpoint'],re['']  ))
        print(re['endpoint'])
        print(re['counter'][29:33])
        print(re['counter'][42:])
        print(re['Values'][0]['timestamp'])
        print(re['Values'][0]['value'])
        cursor.execute(sql, (re['endpoint'],re['counter'][29:33], re['counter'][41:],re['Values'][0]['timestamp'],re['Values'][0]['value']))
